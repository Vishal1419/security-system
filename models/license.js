...
module.exports.getUsed = (callback) => {
  License.find({is_used: true}, (err, licenses) => {
    if (err) return callback(err, null);
    callback(null, licenses);
  });
};
...
module.exports.getUnused = (callback) => {
  License.find({is_used: false}, (err, licenses) => {
    if (err) return callback(err, null);
    callback(null, licenses);
  });
};
...
module.exports.getUpdated = (callback) => {
  License.find({updated_times: { $gt : 0 }}, (err, licenses) => {
    if (err) return callback(err, null);
    callback(null, licenses);
  });
};
...
module.exports.getByKey = (key, callback) => {
  License.find({key: new RegExp('^'+key+'$', "i")}, (err, licenses) => {
    if(err) return callback(err, null);
    callback(null, licenses);
  });
};
